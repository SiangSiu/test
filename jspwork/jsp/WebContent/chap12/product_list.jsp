...
if(wheregroup  > 1) {
	%>
	[<a href="product_list.jsp?gogroup=1&cat<%=ca %>&pname=<%=pn %>">처음</a>]
	[<a href="product_list.jsp?gogroup=<%=priorgroup %>&cat<%=ca %>&pname=<%=pn %>">이전</a>]
	<% 
}else {
	out.println("[처음]");
	out.println("[이전]");
...
if(wheregroup < totalgroup) { %>
	[<a href="profuct_list.jsp?gogroup=<%=nextgroup %>&cat=<%=ca %>&pname=<%=pn %>">다음</a>]	
	[<a href="profuct_list.jsp?gogroup=<%=totalgroup %>&cat=<%=ca %>&pname=<%=pn %>">마지막</a>]	
